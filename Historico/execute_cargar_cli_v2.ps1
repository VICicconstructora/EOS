#!/usr/bin/env pwsh
# Ejecuta los 40 archivos CARGAR_*.sql via Supabase CLI (v2 - usando query)
...
Write-Host "============================================================"
Write-Host "EJECUTAR 40 archivos CARGAR_*.sql via Supabase CLI (v2)"
Write-Host "============================================================"
Write-Host ""
...
foreach ($idx in 0..($files.Count - 1)) {
    ...
    try {
        # Leer contenido y ejecutar via CLI
        $sql_content = Get-Content $file.FullName -Raw

        # Ejecutar usando stdin pipe
        $sql_content | & $SUPABASE_EXE db query 2>&1 | Out-Null
