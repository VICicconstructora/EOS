---
tipo: proyecto
version: 1.1
creado: 2026-05-09
ultima_actualizacion: 2026-05-09
actualizado_por: JPM
estado: construccion
---

# Well

**Tipo:** Socio
**Fuente de datos real:** Flujo Histórico (Historico.xlsx — no existe en CRM Sinco)
**Código PPTO:** Well
**IDs ERP:** N/A (proyecto socio)

## Identificación

| Campo | Valor |
|-------|-------|
| Ciudad | Bogotá D.C. |
| Localidad | Suba |
| Barrio | Pasadena |
| Dirección | Calle 101 # 45A-90 |
| Estrato | [confirmar] |
| Modalidad | No VIS — Apartamento (concepto coliving/inversión) |
| Número de etapas | 1 |
| Etapa(s) activa(s) | Única |
| Gerencia / Estructuración | DOM Real Estate (Dom Urbanum) |

## Equipo de IC en el proyecto

| Rol | Persona / Empresa |
|-----|------------------|
| Contacto IC | [por definir] |
| Gerente Financiero | [Juan José Leal](../../personas/juan-jose-leal.md) |
| Gerente Jurídico | [Nataly Vinchira](../../personas/nataly-vinchira.md) |
| Gerencia / Estructuración | DOM Real Estate (Dom Urbanum) |
| IC rol | Constructor |
| Diseñador | SYG Arquitectos |

## Características del producto

| Campo | Valor |
|-------|-------|
| Total unidades | [confirmar con Histórico.xlsx] |
| Tipología Apartaestudio | desde 28 m² |
| Tipología Apartaestudio Lockoff | [área confirmar] |
| Tipología Skyloft | hasta ~49 m² |
| Amenidades | Zona de lectura, coworking, jardín central, pet spa, lavandería, gym, yoga lounge, juice bar, cooking lounge, BBQ |
| Precio desde | $338,500,000 |
| Rendimiento renta corta | hasta 35% sobre arriendo tradicional |

## Documentos del proyecto

- [Cronograma de obra](./cronograma-obra.md)
- [Cronograma de programación intermedia](./cronograma-intermedia.md)
- [Cronograma del director](./cronograma-director.md)
- [Inventario de unidades](./unidades.md)
- [Casos legales](./casos-legales.md)

## Notas de contexto

IC actúa como constructor; DOM Real Estate (Dom Urbanum) lleva la gerencia, estructuración y comercialización. Proyecto de concepto coliving/inversión en Pasadena (Suba), orientado a renta corta. Único proyecto sin etapas múltiples en catálogo PPTO. Datos de ejecución en Historico.xlsx.

## Datos Operativos (Datamart)

_Fuente: Datamart.xlsx — actualizado 2026-05-10_

**Director de Proyecto:** DIEGO BENAVIDES

### Etapas

| Etapa | Estado | Fiducia | Crédito | Monto | Venc. Crédito | Ventas Proy. |
|-------|--------|---------|---------|-------|---------------|-------------|
| E1 | ACTIVO | Alianza Fiduciaria | BANCO DE BOGOTA | $29.2B | 2027-04-16 | $52.8B |

### Pólizas

| Etapa | Entidad TR | Venc. TR | Entidad RC | Venc. RC |
|-------|------------|----------|------------|----------|
| E1 | Seguros Mundial | 2026-05-14 🟡 | Seguros Mundial | 2027-05-15 |

### Licencias

| Etapa | Lic. Urbanismo | Lic. Construcción | Venc. Lic. Const. | Próx. Trámite |
|-------|----------------|-------------------|-------------------|---------------|
| E1 | 11001-1-21-4777 | 11001-1-21-4777 | 2025-09-26 🔴 | 2026-09-23 |

### Alarmas

- 🟡 **Póliza TR POR VENCER:** E1: Seguros Mundial, vence 2026-05-14 (4 días)
- 🔴 **Licencia Construcción VENCIDA:** E1: venció 2025-09-26 (-226 días)
## Changelog

| Versión | Fecha | Cambio | Autor |
|---------|-------|--------|-------|
| 1.0 | 2026-05-09 | Creación inicial | JPM |
| 1.1 | 2026-05-09 | Datos reales: Pasadena Suba Calle 101 #45A-90, DOM Real Estate, SYG Arquitectos, 28-49m², tipologías Apartaestudio/Lockoff/Skyloft, $338.5M, renta corta | JPM |
