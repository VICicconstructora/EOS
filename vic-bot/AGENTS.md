...
- **Primario por defecto: NVIDIA (OpenAI-compatible), que es gratis.** Cubre a todo el que no haya registrado una key propia de Anthropic. Cada persona puede registrar su `nvapi-...` con `/registrar-nvidia` para no depender de la cuota compartida.
- **Anthropic (Claude) solo con key propia.** Quien registró su `sk-ant-...` con `/registrar-key` lo tiene de primario — es su cuota y su decisión —, con NVIDIA de respaldo. Quien no la registró **nunca** llega a Anthropic.
- **La key compartida de Anthropic está apagada (`VIC_SHARED_ANTHROPIC`, default off).** *(Cambio 2026-09-11: ese día se agotó el saldo completo en una jornada. No fue el volumen — 74 preguntas — sino el costo unitario: sin prompt caching cada iteración reenvía `SYSTEM` + 26 herramientas (~8.000 tokens de piso) y una consulta de cartera contra SINCO encadena hasta 10 iteraciones de Opus; una sola llegó a 1.085.958 tokens de entrada. Seis personas sin key propia caían ahí por el fallback silencioso.)* Ponerla en `1` la revive, y solo tiene sentido junto con prompt caching y un modelo más barato.
- **Contrapartida asumida:** NVIDIA escribiendo SQL contra SINCO ya produjo cifras erradas (cartera 8x, facturación 6x en ago-2026) y fue la razón de que Anthropic pasara a primario el 2026-08-30. Con el default invertido, **las consultas numéricas de negocio exigen key propia de Anthropic**; por NVIDIA, tratar las cifras como indicativas.
...
- **El fallback no es silencioso.** Si la persona registró su key y esa key falla, VIC contesta por el respaldo y le avisa **una vez por conversación** qué pasó con la suya. Sin el aviso, el fallback la deja creyendo que consume su propia cuota (le pasó a Pablo Ángel el 2026-09-11: registró su `sk-ant-`, VIC le respondió por NVIDIA y nada lo indicaba). Y si fallan todos, se lanza el error **más accionable** (un 4xx del primario), no el último: un 500 pasajero del respaldo tapaba el "prompt demasiado largo" o el "key revocada" que sí dicen qué hacer.
- **Cada resultado de tool tiene tope (`VIC_TOOL_RESULT_MAX_CHARS`, 40.000 chars).** Un `get_wiki_page` sobre un Excel o un PDF convertido trae megabytes y el agentic loop los apila: el 2026-09-11 una consulta llegó a **1.085.958 tokens contra el límite de 1.000.000** de Claude y la API devolvió 400. El corte lleva una nota explícita al modelo ("no es el final del documento, acota") para que no concluya que el dato no existe.
...
> **Config (`.env` raíz):** `VIC_OPENAI_API_KEY`, `VIC_OPENAI_BASE_URL` (default `https://integrate.api.nvidia.com/v1`), `VIC_OPENAI_MODEL`, `VIC_OPENAI_TEMPERATURE`, `VIC_OPENAI_TOP_P` para el primario; `VIC_ANTHROPIC_API_KEY` (o `ANTHROPIC_API_KEY`) para la compartida de Claude, que solo se lee si `VIC_SHARED_ANTHROPIC=1`. Opcionales: `VIC_ANTHROPIC_MODEL` (hoy `claude-opus-5`), `VIC_ANTHROPIC_MAX_TOKENS` (default **16000**), `VIC_MAX_TOKENS` (NVIDIA, default **8000**), `VIC_MAX_ITERATIONS` (default 10). `VIC_DEFAULT_PROVIDER` quedó obsoleta el 2026-09-11: el orden ya no se invierte por env, lo decide la key propia de quien pregunta. El adaptador OpenAI usa `fetch` nativo (Node ≥18), sin añadir dependencias.
...
| `VIC_ANTHROPIC_API_KEY` | Key **compartida** de Claude. Si falta, se usa `ANTHROPIC_API_KEY` del `.env` raíz. ⚠ **Ignorada salvo que `VIC_SHARED_ANTHROPIC=1`**: por defecto nadie cae en ella, y quien no registró su `sk-ant-` responde con NVIDIA. |
| `VIC_SHARED_ANTHROPIC` | Interruptor de la key compartida de Claude. Default **off**. En `1` (o `true`) vuelve a cubrir a quien no registró la suya — el comportamiento que vació el saldo el 2026-09-11. Encenderla sin prompt caching y sin un modelo más barato repite el gasto. |
